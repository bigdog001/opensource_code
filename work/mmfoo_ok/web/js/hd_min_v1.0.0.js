var loadingProcess = {isReady:false,url:"",init:function() {
    ...
    picShow.setTit();
    (picShow.isIE ? loadingProcess.thisMovie("picMe") : loadingProcess.thisMovie("picMeFF")).picLoader(a)
},loadPic:function() {
    var a = new Image;
    a.src = this.url;
    if (a.complete) {
...
picShow = {$:function(a) {
...
},isIE:navigator.appVersion.indexOf("MSIE") != -1 ? true : false,isFrist:false,Src:[],Img_prv:new Image,Img_Next:new Image,Img:new Image,Mark:"p",defatLink:"http://www.qq.com/",SiteName:"www",ops:0,opsE:70,Count:photoJson.length,Now:0,imgW:570,imgH:380,pross:0,lastUrl:"",timerMe:null,titleStr:document.title,posX:null,posY:null,Cleft:0,Mleft:0,niersenPath:"/niersen_tongji.htm",tmpLeft:0,tmpRight:0,tmpType:'down',Display:function(a, b) {
    this.Now = Number(b);
    ...
    if (a == "open") {
        ...
        this.$("rePlay").href = "#";
        addEventHandler(picShow.$("rePlay"), "click", function(e) {
        ...
        addEventHandler(picShow.$("rePlay"), "focus", function() {
            picShow.$("rePlay").blur()
        });
        if (picShow.isIE) {
    ...
    if (this.Count > 6) {
        ...
        addEventHandler(picShow.$("scrollbar-in"), "mousedown", function(b) {
    ...
    addEventHandler(picShow.$("preArrow"), "mousemove", function() {
        picShow.$("preArrow").style.cursor = "url(http://mat1.gtimg.com/news/2009hd/arr_left.cur),auto"
    });
    addEventHandler(picShow.$("nextArrow"), "mousemove", function() {
        picShow.$("nextArrow").style.cursor = "url(http://mat1.gtimg.com/news/2009hd/arr_right.cur),auto"
    });
    addEventHandler(picShow.$("preArrow"), "click", function() {
        picShow.viewPrev()
    });
    addEventHandler(picShow.$("nextArrow"), "click", function() {
        picShow.viewNext()
    });
    addEventHandler(picShow.$("Up"), "click", function() {
        picShow.viewPrev()
    });
    addEventHandler(picShow.$("Down"), "click", function() {
        picShow.viewNext()
    });
    ...
    addEventHandler(document, "keydown", function(b) {
        b = window.event || b;
    ...
    var a = window.document.location.href,b = a.indexOf("/");
    ...
    var b = this.Now,c = Math.ceil((b + 1) / 6),d = Number(this.Count);
    (c < 1 || b >= d) && (c = 1);
    if (a == "next") {
        if (c * 6 < this.Count) {
            this.viewJump(c * 6);
    ...
    var b = this.$(a).innerHTML = "",c = document.createElement("li");
    parseInt(this.Now);
    for (var d = Number(this.Count),f = new Image,e = 0; e < d; e++) {
        f.src = photoJson[e].smallpic;
        c.innerHTML = f.width > f.height ? "<div onclick='picShow.viewJump(" + e + ")'><span class='imgs'><img src=" + photoJson[e].smallpic + " width='78' alt='" + (e + 1) + "'/></span><span class='titles'>" + (e + 1) + "<em>&nbsp;/ " + d + "</em></span></div>" : "<div onclick='picShow.viewJump(" + e + ")'><span class='imgs'><img src=" + photoJson[e].smallpic + " alt='͵" + (e + 1) + "ͼƬ' /></span><span class='titles'>" + (e + 1) + "<em>&nbsp;/ " + d + "</em></span></div>";
